...
# Hint 1: We can schedule all type 2 operations before type 1 operations.
# Explanation:
# Type 2 operations flip any character, while type 1 operations only change 
# the relative order of characters without altering their values. Since 
# type 1 operations do not modify character values, we can first perform all 
# necessary type 2 operations to fix the characters, and then use type 1 
# operations to adjust their positions. Therefore, without loss of generality, 
# we may assume that all type 2 operations are performed first.
