...
# We precompute prefix sums for each row and each column of the matrix grid.
# Each row sum can be computed in O(1) time using row prefix sums. 
#   Since there are edge rows, the total time is O(edge).
# Each column sum can be computed in O(1) time using column prefix sums. 
#   Since there are edge columns, the total time is O(edge).
# We do not preprocess diagonal prefix sums because there are only two diagonals. 
#   Computing them directly still takes O(edge) time.
# As a result, the total time for checking one square is reduced from O(edge^2) to O(edge).
